...
public class ClickStreamThree {
	...
	extends Mapper<LongWritable, Text, Text, WebLogBean> {
		...
		protected void map(LongWritable key,Text value,Context context) 
			...
			v.set("true".equals(fields[0]) ? true : false, 
	...
	static class ClickStreamReducer 
		extends Reducer<Text, WebLogBean, NullWritable, Text> {
		Text v = new Text();
		@Override
		protected void reduce(Text key, Iterable<WebLogBean> values, 
			...
			try {
				...
				/**
				 * 以下逻辑为：从有序bean中分辨出各次visit，并对一次visit中所访
				 * 问的page按顺序标号step
				 */
				int step = 1;
				...
				for (int i = 0; i < beans.size(); i++) {
					...
					if (1 == beans.size()) {
						// 设置默认停留市场为60s
						v.set(session+"\001"+key.toString()+"\001"+
						bean.getRemote_user() + "\001" + 
						bean.getTime_local() + "\001" + 
						bean.getRequest() + "\001" + step + "\001" + 
						(60) + "\001"+bean.getHttp_referer()+"\001"+ 
					...
					// 求近两次时间差
					long timeDiff = 
						timeDiff(toDate(bean.getTime_local()), 
					...
					if (timeDiff < 30 * 60 * 1000) {
						...
						beans.get(i - 1).getRequest() + "\001" + step+ 
						"\001" + (timeDiff / 1000) + "\001" + 
		...
		private long timeDiff(String time1, String time2) 
